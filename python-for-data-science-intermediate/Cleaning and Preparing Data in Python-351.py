...
def strip_characters(string: str):
    ...
    for ch in string:
        if ord(ch) != 45 and (ord(ch) < 48 or ord(ch) > 57):
...
def process_date(string: str):
    
    if '-' in string:
        dash_index = string.index('-')
        before = int(string[0 : dash_index])
        end = int(string[dash_index+1 : ])
        
        new_date = round((before+end)/2)
    else:
        new_date = int(string)
    return new_date
